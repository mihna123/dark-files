...
func load_fs():
	...
	for file_obj in fs_array:
		if file_obj.desktop:
			add_file(file_obj.file_id, file_obj.file_name, file_obj.text_content, file_obj.sub_files, file_obj.password)

func add_file(file_id: int,file_name: String, text_contents: String, subfs: Array, password: String):
